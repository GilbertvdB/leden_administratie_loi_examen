<div>
    ...
    <div class="relative w-full h-full max-w-md md:h-auto">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700 border-2 border-sky-500">
            ...
            <form id="del-form" method="POST" action="{{ $action }}" class="pl-4">
            <input type="hidden" name="user-id" id="user-id" value="" ><br>
            @csrf
            @method('delete')
...
for (var i=0; i<buttons.length; ++i) {
  buttons[i].addEventListener('click', setFormHiddenValue);
}

function setFormHiddenValue() {
  document.getElementById("user-id").value = (this.id); 
}
